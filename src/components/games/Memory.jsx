...
export default function Memory({ room, user, myRole }) {
    ...
    const checkMatch = async () => {
        ...
        if (card1.val === card2.val && card1.suit === card2.suit) {
            ...
            const scoreField = myRole === 'host' ? "gameData.hostScore" : "gameData.guestScore";
            const currentScore = myRole === 'host' ? gd.hostScore : gd.guestScore;
            const leftPairs = (gd.pairsLeft || 10) - 1; 
            ...
            if (leftPairs === 0) {
                ...
                if (winner !== 'draw') {
                    await addDoc(collection(db, "history"), { uid: winner, type: "게임", msg: `${t.g_memory} Win`, amount: room.betAmount * 2, createdAt: serverTimestamp() });
                }
                setProcessing(false);
                return;
            } else {
    ...
    const endGame = async (winnerUid, reason) => {
        ...
        await addDoc(collection(db, "history"), { uid: winnerUid, type: "게임", msg: `${t.g_memory} ${reason}`, amount: room.betAmount * 2, createdAt: serverTimestamp() });
    };
    ...
    const restartGame = async () => {
        ...
        try {
            ...
            await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: "Memory Restart", amount: -room.betAmount, createdAt: serverTimestamp() });
    ...
    if (!gd.deck) return <div>Setting up...</div>;
    ...
    return (
        <div style={{textAlign:'center', width:'100%', position:'relative'}}>
            ...
            <div style={{background:'#2c3e50', padding:10, borderRadius:10, marginBottom:15, display:'flex', justifyContent:'space-between', alignItems:'center', border:'2px solid #555'}}>
                <div style={{opacity: gd.turn === room.host ? 1 : 0.5}}>
                    <div style={{color:'#f1c40f', fontWeight:'bold'}}>{room.hostName}</div>
                    <div style={{fontSize:24, color:'white'}}>{gd.hostScore}</div>
                </div>
                <div style={{color:'white', fontWeight:'bold'}}>
                    {!isFinished && <span style={{color: timeLeft<=5?'#e74c3c':'#fff'}}>⏳ {timeLeft}s</span>}
                </div>
                <div style={{opacity: gd.turn === room.guest ? 1 : 0.5}}>
                    <div style={{color:'#f1c40f', fontWeight:'bold'}}>{room.guestName}</div>
                    <div style={{fontSize:24, color:'white'}}>{gd.guestScore}</div>
                </div>
            </div>
            ...
            {isFinished && (
                <div style={{position:'absolute', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.85)', zIndex:100, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', borderRadius:10}}>
                    <h1 style={{color: room.winner === user.uid ? '#f1c40f' : '#ccc', fontSize:40, marginBottom:10}}>
                        {room.winner === user.uid ? t.win : (room.winner === 'draw' ? t.draw : t.lose)}
                    </h1>
                    <div style={{color:'white', fontSize:20, marginBottom:30}}>
                        {room.hostName}: {gd.hostScore} vs {room.guestName}: {gd.guestScore}
                    </div>
                    ...
                    <div style={{display:'flex', gap:10}}>
                        {myRole === 'host' ? (
                            ...
                            <>
                                {!isGuestReady ? (
                                    <button className="btn" style={{background:'#27ae60', padding:'15px', fontWeight:'bold'}} onClick={handleGuestReady}>{t.ready}</button>
                                ) : (
                                    <div style={{padding:'15px', background:'#2c3e50', color:'#ccc', borderRadius:5}}>{t.waitHost}</div>
                                )}
                                <button className="btn" style={{background:'#555', padding:'15px', fontWeight:'bold'}} onClick={handleGuestExit}>{t.exit}</button>
                            </>
                        )}
                    </div>
                </div>
