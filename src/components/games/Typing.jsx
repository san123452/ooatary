...
import React, { useState, useEffect } from 'react';
import { db, auth } from '../../firebase';
import { doc, updateDoc, increment, runTransaction, onSnapshot, collection, addDoc, serverTimestamp, deleteDoc } from 'firebase/firestore';
import { useNavigate, useParams } from 'react-router-dom';
import { onAuthStateChanged } from 'firebase/auth';
...
export default function Typing() {
    ...
    useEffect(() => {
        ...
        return () => clearInterval(timer);
    }, [input, gameResult, room]); 

    const handleTimeOut = () => {
        const winnerUid = user.uid === room.host ? room.guest : room.host;
        handleWin(winnerUid, "Time Out");
    };
    ...
    const handleWin = async (winnerUid, reason = "Win") => {
        if (room.status === 'finished') return;
        try {
            await runTransaction(db, async (t) => {
                const roomRef = doc(db, "battle_rooms", roomId);
            ...
            await addDoc(collection(db, "history"), {
                uid: winnerUid, type: "게임", msg: `${t.g_typing} ${reason}`, amount: room.betAmount * 2, createdAt: serverTimestamp()
            });
    ...
    const restartGame = async () => {
        ...
        try {
            await runTransaction(db, async (t) => {
                ...
                if (hostSnap.data().point < room.betAmount || guestSnap.data().point < room.betAmount) throw new Error("No Money");
            ...
            await addDoc(collection(db, "history"), {
                uid: user.uid, type: "게임", msg: "Typing Restart", amount: -room.betAmount, createdAt: serverTimestamp()
            });
        } catch (e) {
    ...
    if (!roomId || !user || !room) return <div style={{background:'#2c3e50', minHeight:'100vh', color:'white', textAlign:'center', paddingTop:50}}><h1>{t.loading}</h1></div>;

    const targetSentence = room.gameData?.sentence || "...";

    return (
        <div className="container" style={{ background: '#2c3e50', minHeight: '100vh', padding: '20px', color: 'white', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <h1 style={{ color: '#f1c40f', marginBottom: 20 }}>⌨️ {t.g_typing}</h1>
            
            <div className="card" style={{ background: '#34495e', padding: '10px', marginBottom: 20, width: '100%', maxWidth: '600px' }}>
                <div style={{ fontSize: 18 }}>Pot: <span style={{ color: '#2ecc71', fontWeight: 'bold' }}>{room.betAmount?.toLocaleString()}</span></div>
                {!gameResult && <div style={{ color: timeLeft<=5?'#e74c3c':'#f1c40f', fontWeight:'bold', marginTop:5 }}>Time: {timeLeft}s</div>}
            </div>
            ...
            <div style={{ width: '100%', maxWidth: '600px', marginBottom: '30px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                    <span style={{ fontWeight: 'bold', color: isError ? '#e74c3c' : '#3498db' }}>
                        😎 {isHost ? room.hostName : room.guestName} ({myProgress}%) {isError && "❌Miss!"}
                    </span>
                </div>
                <div style={{ width: '100%', height: '20px', background: '#555', borderRadius: '10px', overflow: 'hidden', marginBottom: '15px' }}>
                    <div style={{ width: `${myProgress}%`, height: '100%', background: isError ? '#e74c3c' : '#3498db', transition: 'width 0.2s' }}></div>
            ...
            <textarea 
                ...
                onChange={handleChange} 
                disabled={gameResult !== null} 
                placeholder={gameResult ? "Game Over" : "Type here!"} 
                className="input" 
                style={{ 
                    width: '100%', maxWidth: '600px', padding: '15px', fontSize: '18px', textAlign: 'left', 
                    background: gameResult ? '#7f8c8d' : (isError ? '#fab1a0' : 'white'), 
                        ...
                        {isHost ? (
                            ...
                            <>
                                {!isGuestReady ? (
                                    ...
                                    <div style={{ color: '#ccc', fontSize: '18px', display: 'flex', alignItems: 'center' }}>{t.waitHost}</div>
                                )}
                                <button className="btn" style={{ fontSize: '20px', padding: '15px 30px', background: '#555' }} onClick={handleGuestExit}>
                                    {t.exit}
                                </button>
