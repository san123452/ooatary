...
const styles = `
  @keyframes shake-hard { 0% { transform: translate(0, 0) rotate(0deg); } 25% { transform: translate(5px, 5px) rotate(5deg); } 50% { transform: translate(0, 0) rotate(0deg); } 75% { transform: translate(-5px, -5px) rotate(-5deg); } 100% { transform: translate(0, 0) rotate(0deg); } }
  @keyframes glow { 0% { box-shadow: 0 0 10px rgba(255,255,255,0.5); } 100% { box-shadow: 0 0 30px rgba(255,255,255,1); } }
  .capsule-modal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); z-index: 1000; display: flex; justify-content: center; align-items: center; backdrop-filter: blur(5px); }
  .capsule-content { background: #2f3640; padding: 40px; border-radius: 20px; text-align: center; border: 2px solid #555; max-width: 90%; width: 400px; }
  .machine-glass { background: linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0) 100%); border: 1px solid rgba(255,255,255,0.3); box-shadow: inset 0 0 20px rgba(255,255,255,0.1); }
`;
...
export default function Mining() {
  ...
  const startTimer = (seconds) => {
    ...
    timerRef.current = setInterval(() => {
      timeLeft -= 1;
      setCooldown(timeLeft);
      if (timeLeft <= 0) { clearInterval(timerRef.current); localStorage.removeItem(STORAGE_KEY); }
    }, 1000);
  };
  ...
  const playGacha = async () => {
    ...
    setTimeout(async () => {
      const rand = Math.random() * 100;
      let prize = 0; let tier = ""; let color = ""; let msg = "";

      if (rand < 0.1) { prize = 1000000000; tier = "GOD"; color = "#ff00ff"; msg = t.g_god; } 
      else if (rand < 1.1) { prize = 100000000; tier = "MYTHIC"; color = "#000"; msg = t.g_mythic; } 
      else if (rand < 6.1) { prize = 5000000; tier = "LEGEND"; color = "#00d2d3"; msg = t.g_legend; } 
      else if (rand < 16.1) { prize = 1000000; tier = "EPIC"; color = "#f1c40f"; msg = t.g_epic; } 
      else { prize = Math.floor(Math.random() * 50001) + 50000; tier = "COMMON"; color = "#ff4757"; msg = t.g_common; }
  ...
  const closeModal = () => { setShowModal(false); setPrizeData(null); };
  const progress = Math.min(100, (cooldown / 60) * 100);

  return (
    <div className="container" style={{ textAlign: 'center', paddingTop: 30, background: '#131517', minHeight: '100vh', color: 'white', fontFamily: 'sans-serif' }}>
      <style>{styles}</style>
      <h1 className="title" style={{ background: 'linear-gradient(to right, #f1c40f, #e67e22)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontSize: '28px', marginBottom: '10px' }}> {t.gachaTitle} </h1>
      
      <div style={{ background: 'rgba(255,255,255,0.05)', display: 'inline-block', padding: '10px 25px', borderRadius: '50px', border: '1px solid #333', fontSize: '20px', fontWeight: 'bold', color: '#f1c40f', marginBottom: '30px' }}>
        💎 {point.toLocaleString()} P
      </div>

      <div style={{ position: 'relative', width: '280px', margin: '0 auto' }}>
        <div className="machine-glass" style={{ width: '280px', height: '280px', borderRadius: '50%', background: '#222', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', border: '8px solid #333', boxShadow: '0 0 30px rgba(0,0,0,0.5)' }}>
            <div style={{ fontSize: '120px', filter: isLoading ? 'blur(2px)' : 'none', transition: '0.3s', animation: isLoading ? 'shake-hard 0.5s infinite' : 'none' }}>
                {cooldown > 0 ? '🔒' : '🌏'}
            </div>
            {cooldown > 0 && (
                <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.7)', borderRadius: '50%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backdropFilter: 'blur(3px)' }}>
                    <div style={{ fontSize: '40px', fontWeight: 'bold', color: '#e74c3c' }}>{cooldown}</div>
                    <div style={{ fontSize: '14px', color: '#aaa' }}>{t.g_wait}</div>
                </div>
            )}
        </div>

        <button onClick={playGacha} disabled={cooldown > 0 || isLoading} style={{ marginTop: '-40px', position: 'relative', zIndex: 10, width: '220px', height: '70px', background: cooldown > 0 ? '#555' : 'linear-gradient(90deg, #e67e22, #f1c40f)', border: 'none', borderRadius: '15px', color: 'white', fontSize: '20px', fontWeight: 'bold', boxShadow: cooldown > 0 ? 'none' : '0 10px 0 #d35400, 0 10px 20px rgba(0,0,0,0.4)', cursor: cooldown > 0 ? 'not-allowed' : 'pointer', transition: 'all 0.1s', transform: isLoading ? 'translateY(10px)' : 'translateY(0)' }}>
          {isLoading ? "..." : cooldown > 0 ? "WAIT" : t.g_pull}
        </button>
      ...
      {showModal && prizeData && (
        <div className="capsule-modal" onClick={closeModal}>
          <div className="capsule-content" onClick={(e) => e.stopPropagation()}>
            <div style={{ fontSize: '14px', color: '#aaa', letterSpacing: '2px', marginBottom: '10px' }}>CONGRATULATIONS</div>
            <div style={{ width: '120px', height: '120px', borderRadius: '50%', margin: '0 auto 20px', background: prizeData.color, boxShadow: `0 0 30px ${prizeData.color}`, animation: 'glow 1.5s infinite alternate', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '50px' }}>
               🎁
            </div>
            <h2 style={{ margin: '10px 0', fontSize: '24px', color: prizeData.color }}>{prizeData.msg}</h2>
            <div style={{ fontSize: '32px', fontWeight: 'bold', color: 'white', margin: '15px 0' }}>+{prizeData.money.toLocaleString()}</div>
            <button className="btn" onClick={closeModal} style={{ width: '100%', background: '#fff', color: '#000', fontWeight: 'bold', padding: '15px', borderRadius: '10px' }}>{t.confirm}</button>
          </div>
        </div>
      )}
      <button className="btn" style={{ marginTop: 20, background: 'transparent', border:'1px solid #444', color:'#888' }} onClick={() => navigate('/home')}> {t.back} </button>
    </div>
  );
}