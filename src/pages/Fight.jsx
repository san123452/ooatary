...
export default function Fight() {
  ...
  const [red, setRed] = useState({ hp: 100, action: '', effect: null });
  const [blue, setBlue] = useState({ hp: 100, action: '', effect: null });
  const [log, setLog] = useState("Select Fighter!");
  const timerRef = useRef(null);
  const navigate = useNavigate();
  const user = auth.currentUser;
  const { t } = useLanguage();
  ...
  const startFight = async () => {
    const betMoney = parseInt(bet);
    if (!selectedSide) return alert("Select side!");
    ...
    try {
      ...
      await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: `파이트 배팅`, amount: -betMoney, createdAt: serverTimestamp() });
  ...
  const runGameLoop = (betMoney) => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (isRedAttacking) { setRed(prev => ({ ...prev, action: 'attack-right' })); setTimeout(() => setRed(prev => ({ ...prev, action: '' })), 200); if (isDodge) { setBlue(prev => ({ ...prev, effect: 'MISS 💨' })); setLog("BLUE Dodge!"); } else { bHP = Math.max(0, bHP - damage); setBlue(prev => ({ hp: bHP, action: 'hit', effect: isCrit ? `CRITICAL! -${damage}` : `-${damage}` })); setLog(isCrit ? `💥 RED CRIT!!` : `👊 RED ATTACK!`); } } 
      else { setBlue(prev => ({ ...prev, action: 'attack-left' })); setTimeout(() => setBlue(prev => ({ ...prev, action: '' })), 200); if (isDodge) { setRed(prev => ({ ...prev, effect: 'MISS 💨' })); setLog("RED Dodge!"); } else { rHP = Math.max(0, rHP - damage); setRed(prev => ({ hp: rHP, action: 'hit', effect: isCrit ? `CRITICAL! -${damage}` : `-${damage}` })); setLog(isCrit ? `💥 BLUE CRIT!!` : `👊 BLUE ATTACK!`); } }
  ...
  const finalizeGame = async (winner, betMoney) => {
    ...
    if (isWin) {
      const prize = betMoney * 1.98;
      setLog(`🎉 WINNER: ${winner.toUpperCase()} (+${prize.toLocaleString()})`);
      try {
        await updateDoc(doc(db, "users", user.uid), { point: increment(prize) });
        setPoint(prev => prev + prize);
        await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: "파이트 승리", amount: prize, createdAt: serverTimestamp() });
      } catch(e) {}
    } else {
      setLog(`💀 WINNER: ${winner.toUpperCase()}`);
    }
  };
  ...
  return (
    <div className="container" style={{ background: '#131517', minHeight: '100vh', padding: '20px', color: 'white', textAlign: 'center' }}>
      ...
      <div className="arena" style={{ marginBottom: 20 }}>
        ...
        <div className="fighter-container" style={{ left: '15%', ...getAnim('red', red) }}> {red.hp <= 0 ? ( <div style={{ fontSize: '80px' }}>😵</div> ) : ( <img src={`/fight/${redCharId}.png`} className="fighter-img" alt="Red" style={{ filter: 'drop-shadow(0 0 10px rgba(255,0,0,0.3))' }} onError={(e) => {e.target.style.display='none'}} /> )} {red.effect && ( <div className={`damage-text ${red.effect.includes('CRIT') ? 'crit-text' : ''}`}> {red.effect} </div> )} </div>
        <div className="fighter-container" style={{ right: '15%', ...getAnim('blue', blue) }}> {blue.hp <= 0 ? ( <div style={{ fontSize: '80px' }}>😵</div> ) : ( <img src={`/fight/${blueCharId}.png`} className="fighter-img" alt="Blue" style={{ transform: 'scaleX(-1)', filter: 'drop-shadow(0 0 10px rgba(0,0,255,0.3))' }} onError={(e) => {e.target.style.display='none'}} /> )} {blue.effect && ( <div className={`damage-text ${blue.effect.includes('CRIT') ? 'crit-text' : ''}`}> {blue.effect} </div> )} </div>
        <div style={{ position: 'absolute', bottom: 20, width: '100%', borderBottom: '4px solid #444' }}></div>
      </div>
      <div className="card" style={{ background: '#222', color: '#f1c40f', minHeight: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 'bold', border: '1px solid #444', marginBottom: 20 }}> {log} </div>
      {gameState !== 'fighting' && (
        <div style={{ animation: 'bounce-in 0.5s' }}>
          <div className="card" style={{ background: '#1e272e', padding: '20px' }}>
            <div style={{ fontSize: 18, marginBottom: 15, color: '#ccc' }}> {t.balance}: <span style={{ color: '#f1c40f', fontWeight: 'bold' }}>{Math.floor(point).toLocaleString()}</span> </div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
                <button className="btn" style={{ flex: 1, background: selectedSide === 'red' ? '#ff4757' : '#444', height: 60, fontSize: 18, border: selectedSide === 'red' ? '3px solid white' : 'none' }} onClick={() => setSelectedSide('red')}> 👹 RED (x1.98) </button>
                <button className="btn" style={{ flex: 1, background: selectedSide === 'blue' ? '#3742fa' : '#444', height: 60, fontSize: 18, border: selectedSide === 'blue' ? '3px solid white' : 'none' }} onClick={() => setSelectedSide('blue')}> 🤖 BLUE (x1.98) </button>
            </div>
            <input className="input" type="number" placeholder={t.inputBet} value={bet} onChange={e => setBet(e.target.value)} style={{ textAlign: 'center', fontSize: 20, width: '100%', marginBottom: 15 }} />
            <div style={{display:'flex', gap:5, marginBottom:20}}>
                <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#7f8c8d'}} onClick={()=>handleBetPercent(0.1)}>10%</button>
                <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#7f8c8d'}} onClick={()=>handleBetPercent(0.25)}>25%</button>
                <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#7f8c8d'}} onClick={()=>handleBetPercent(0.5)}>50%</button>
            ...
            <button className="btn btn-primary" style={{ width: '100%', padding: '15px', fontSize: '20px', fontWeight: 'bold' }} onClick={startFight}> {t.gameStart} </button>
          </div>
        </div>
      )}
      {gameState !== 'fighting' && ( <button className="btn" style={{ marginTop: 20, background: '#333', width: '100%', padding: '15px' }} onClick={() => navigate('/home')}> {t.home} </button> )}
    </div>
  );
}