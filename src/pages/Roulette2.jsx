...
export default function Roulette2() {
  ...
  const spinWheel = async () => {
    if (selectedBet === null) return alert("어디에 걸지 선택하세요! (색깔 또는 숫자)");
    const money = parseInt(betAmount);
    if (isNaN(money) || money <= 0) return alert("배팅금을 입력하세요!");
    if (money > Math.floor(point)) return alert("돈이 부족합니다!");
    ...
    try {
      ...
      await addDoc(collection(db, "history"), {
          uid: user.uid,
          type: "게임",
          msg: `룰렛 배팅 (${typeof selectedBet === 'number' ? selectedBet : selectedBet.toUpperCase()})`,
          amount: -money,
          createdAt: serverTimestamp()
      });

    } catch (e) { setIsSpinning(false); return alert("오류 발생"); }
  ...
  return (
    ...
    <div className="container" style={{ background: '#1e272e', minHeight: '100vh', color: 'white', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ width: '100%', padding: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#2f3640', zIndex: 10 }}> <h1 style={{ margin: 0, fontSize: '20px', color: '#e74c3c' }}>🎡 ROULETTE</h1> <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#f1c40f' }}>💰 {Math.floor(point).toLocaleString()}</div> </div>
      ...
      {lastResult && ( <div style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '10px', color: lastResult.color === 'red' ? '#e74c3c' : (lastResult.color === 'green' ? '#2ecc71' : '#bdc3c7') }}> 결과: {lastResult.num} ({lastResult.color.toUpperCase()}) </div> )}
      <div className="card" style={{ width: '95%', background: '#2c3e50', padding: '15px', marginBottom: '20px' }}>
         ...
         <input className="input" type="number" placeholder="배팅 금액 입력" value={betAmount} onChange={e => setBetAmount(e.target.value)} style={{ width: '100%', marginBottom: '10px', textAlign: 'center', background: '#1e272e', color: 'white' }} />
         ...
         <button className="btn btn-primary" style={{ width: '100%', padding: '15px', fontSize: '20px', background: isSpinning ? '#555' : '#f1c40f', color: 'black', fontWeight: 'bold' }} onClick={spinWheel} disabled={isSpinning}> {isSpinning ? '돌아가는 중...' : '🎰 SPIN!'} </button>
      </div>
      <button className="btn" style={{ width: '90%', background: '#444' }} onClick={() => navigate('/home')}>🏠 홈으로</button>
    </div>
  );
}