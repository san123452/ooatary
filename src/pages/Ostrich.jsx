...
export default function CrazyOstrich() {
  ...
  const startGame = async () => {
    // 1️⃣ [즉시 차단] 함정 카드 발동! 이미 처리 중이면 아예 읽지도 않습니다.
    if (isProcessing.current || gameState === 'running') return;
    ...
    // 2️⃣ [잠금 및 UI 선반영] DB 통신보다 '버튼 끄기'를 먼저 해서 클릭을 막습니다.
    ...
    for(let i=0; i<fakeCount; i++) { 
      ...
      const deepX = isLeft ? -220 : 220; 
      moves.push({ x: deepX, dir: dir }); 
      moves.push({ x: 0, dir: dir === 'left' ? 'right' : 'left' }); 
    }
    const endX = finalResult === 'left' ? -1500 : 1500;
    moves.push({ x: endX, dir: finalResult });
    ...
    const runMove = () => {
        if (step >= moves.length) { 
          ...
          finishGame(finalResult, money); 
          return; 
        }
        const move = moves[step]; 
        setOstrichX(move.x); 
        if (move.dir) setDirection(move.dir); 
        step++;
    };
    runMove(); 
    timerRef.current = setInterval(runMove, MOVE_DURATION * 1000);
  ...
  const finishGame = async (result, money) => {
    ...
    setTimeout(async () => {
        ...
        // 5️⃣ [잠금 해제] 모든 정산과 알림이 끝난 뒤에만 다음 게임이 가능합니다.
        isProcessing.current = false;
    }, 500);
  };
  ...
  return (
    <div className="container" style={{ background: '#353b48', minHeight: '100vh', color: 'white', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#2f3640', borderBottom: '2px solid #1e272e', zIndex: 20 }}> 
        <h1 style={{ margin: 0, fontSize: '20px', color: '#ff7675' }}>🦩 CRAZY OSTRICH</h1> 
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#5c94fc' }}>
        ...
        <div style={{ position: 'absolute', top: '10%', right: '20%', fontSize: '40px', opacity: 0.6 }}>☁️</div>
        <div style={{ position: 'absolute', bottom: 0, width: '100%', height: '80px', background: 'repeating-linear-gradient(45deg, #e17055 0px, #e17055 20px, #d63031 20px, #d63031 40px)', borderTop: '10px solid #6ab04c' }}></div>
        <div style={{ position: 'absolute', bottom: '80px', left: '10px', fontSize: '30px' }}>🚩 LEFT</div> 
        <div style={{ position: 'absolute', bottom: '80px', right: '10px', fontSize: '30px' }}>RIGHT 🚩</div>
        <div style={{ position: 'absolute', bottom: '90px', left: '50%', fontSize: '80px', transform: `translateX(calc(-50% + ${ostrichX}px)) scaleX(${direction === 'left' ? -1 : 1})`, transition: `transform ${MOVE_DURATION}s linear`, zIndex: 10, textShadow: '0 5px 10px rgba(0,0,0,0.3)' }}> 🦩 </div>
      </div>

      <div className="card" style={{ background: '#2f3640', padding: '20px', borderRadius: '20px 20px 0 0', position: 'relative', zIndex: 10 }}>
        <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
            <button onClick={() => setSelectedSide('left')} disabled={gameState === 'running'} style={{ flex: 1, padding: '20px', borderRadius: '10px', border: 'none', background: selectedSide === 'left' ? '#ff6b6b' : '#7f8c8d', color: 'white', fontSize: '20px', fontWeight: 'bold', boxShadow: selectedSide === 'left' ? '0 5px 0 #c23616' : '0 5px 0 #2d3436', transform: selectedSide === 'left' ? 'translateY(2px)' : 'none', transition: 'all 0.1s' }}> ⬅️ LEFT </button>
            <button onClick={() => setSelectedSide('right')} disabled={gameState === 'running'} style={{ flex: 1, padding: '20px', borderRadius: '10px', border: 'none', background: selectedSide === 'right' ? '#48dbfb' : '#7f8c8d', color: 'white', fontSize: '20px', fontWeight: 'bold', boxShadow: selectedSide === 'right' ? '0 5px 0 #0abde3' : '0 5px 0 #2d3436', transform: selectedSide === 'right' ? 'translateY(2px)' : 'none', transition: 'all 0.1s' }}> RIGHT ➡️ </button>
        </div>
        ...
        <button 
          ...
          style={{ width: '100%', padding: '15px', fontSize: '20px', fontWeight: 'bold', background: (gameState === 'running' || isProcessing.current) ? '#555' : '#1dd1a1', boxShadow: (gameState === 'running' || isProcessing.current) ? 'none' : '0 5px 0 #10ac84', cursor: (gameState === 'running' || isProcessing.current) ? 'not-allowed' : 'pointer' }} 
          onClick={startGame} 
          disabled={gameState === 'running' || isProcessing.current}
        > 
          {gameState === 'running' ? 'Running...' : 'START (x1.97)'} 
        </button>
