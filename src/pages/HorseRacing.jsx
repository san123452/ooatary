...
export default function HorseRacing() {
  ...
  const initialHorses = [ { id: 1, name: 'Red', color: '#e74c3c', pos: 0, odds: 1.3, speedBonus: 1.05 }, { id: 2, name: 'Blue',   color: '#3498db', pos: 0, odds: 2.5, speedBonus: 1.02 }, { id: 3, name: 'Gold', color: '#f1c40f', pos: 0, odds: 3.8, speedBonus: 0.99 }, { id: 4, name: 'Dark', color: '#8e44ad', pos: 0, odds: 6.5, speedBonus: 0.96 }, { id: 5, name: 'Green', color: '#2ecc71', pos: 0, odds: 11.0, speedBonus: 0.92 }, ];
  const [horses, setHorses] = useState(initialHorses);
  const raceInterval = useRef(null);
  const navigate = useNavigate();
  const user = auth.currentUser;
  ...
  const startRace = async () => {
    if (betMoney <= 0) return alert(t.alertInputBet);
    if (betMoney > Math.floor(point)) return alert(t.alertNoMoney);
    if (!selectedHorseId) return alert("Select Horse!");

    await updateDoc(doc(db, "users", user.uid), { point: increment(-betMoney) });
    setPoint(prev => prev - betMoney);
    
    await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: `경마 배팅 (${selectedHorseId})`, amount: -betMoney, createdAt: serverTimestamp() });
    ...
    raceInterval.current = setInterval(() => {
      tick++;
      if (tick === 30) setCommentary("GO! GO!");
      if (tick === 70) setCommentary(".... ....");
      if (tick === 120) setCommentary("FINISH LINE!!");
  ...
  const handleFinish = async (winner) => {
    ...
    if (isWin) {
      const prize = Math.floor(betMoney * winner.odds);
      setCommentary(`🎉 WINNER: ${winner.name} !!`);
      await updateDoc(doc(db, "users", user.uid), { point: increment(prize) });
      setPoint(prev => prev + prize);
      await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: "경마 승리", amount: prize, createdAt: serverTimestamp() });
      if(navigator.vibrate) navigator.vibrate([100,50,100]); 
    } else {
      setCommentary(`WINNER: ${winner.name}...`);
    }
  ...
  const resetGame = () => { setHorses(initialHorses.map(h => ({ ...h, pos: 0 }))); setGameState('betting'); setBetMoney(0); setSelectedHorseId(null); setCommentary("Betting..."); };
  const currentLeaderId = horses.reduce((prev, curr) => (prev.pos > curr.pos ? prev : curr)).id;
  ...
  return (
    <div className="container" style={{ background: '#1e272e', minHeight: '100vh', padding: '10px', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      <div className="card" style={{ background: '#2f3640', padding: '15px', borderTop: '4px solid #f1c40f' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', borderBottom: '1px solid #444', paddingBottom: '10px' }}>
            <div> <div style={{ fontSize: '12px', color: '#aaa' }}>{t.balance}</div> <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#f1c40f' }}>{Math.floor(point).toLocaleString()}</div> </div>
            <div> <div style={{ fontSize: '12px', color: '#aaa' }}>{t.betAmount}</div> <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#e74c3c' }}>{betMoney.toLocaleString()}</div> </div>
        </div>
        {gameState === 'betting' && ( <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '5px', marginBottom: '15px' }}> {horses.map(h => ( <button key={h.id} onClick={() => setSelectedHorseId(h.id)} style={{ background: selectedHorseId === h.id ? h.color : '#222', border: selectedHorseId === h.id ? '2px solid white' : '1px solid #444', borderRadius: '8px', padding: '10px 0', cursor: 'pointer', opacity: selectedHorseId && selectedHorseId !== h.id ? 0.5 : 1 }}> <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#fff' }}>{h.id}</div> <div style={{ fontSize: '11px', color: '#ccc' }}>x{h.odds}</div> </button> ))} </div> )}
        {gameState === 'betting' && ( <div style={{display:'flex', gap:5, marginBottom:15}}> <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#7f8c8d'}} onClick={()=>handleBetPercent(0.1)}>10%</button> <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#7f8c8d'}} onClick={()=>handleBetPercent(0.25)}>25%</button> <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#7f8c8d'}} onClick={()=>handleBetPercent(0.5)}>50%</button> <button className="btn" style={{flex:1, padding:5, fontSize:12, background:'#e74c3c', color:'white', fontWeight:'bold'}} onClick={()=>handleBetPercent(1)}>ALL</button> <button className="btn" style={{width:40, padding:5, fontSize:12, background:'#95a5a6'}} onClick={()=>handleBetPercent(0)}>🔄</button> </div> )}
        {gameState === 'betting' ? ( <button className="btn" onClick={startRace} style={{ width: '100%', padding: '15px', fontSize: '18px', fontWeight: 'bold', background: '#27ae60' }}> START RACE </button> ) : gameState === 'result' ? ( <button className="btn" onClick={resetGame} style={{ width: '100%', padding: '15px', fontSize: '18px', background: '#3498db' }}> {t.restart} </button> ) : ( <button className="btn" disabled style={{ width: '100%', padding: '15px', fontSize: '18px', background: '#555', cursor: 'not-allowed' }}> RACING... </button> )}
      </div>
      <button className="btn" style={{ marginTop: 20, background: 'transparent', border:'1px solid #555', color:'#888', width: '100%' }} onClick={() => navigate('/home')}> {t.home} </button>
    </div>
  );
}