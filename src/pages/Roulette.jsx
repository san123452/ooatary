...
//     if (!selectedSide) return alert(t.rouletteSelect);
...
//     await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: `천사악마 배팅`, amount: -betMoney, createdAt: serverTimestamp() });
...
//         await addDoc(collection(db, "history"), { uid: user.uid, type: "게임", msg: "천사악마 승리", amount: winAmount, createdAt: serverTimestamp() });
...
//       <style>{styles}</style>
//       <h2 style={{ fontFamily: 'fantasy', letterSpacing: '2px', color: '#ecf0f1', marginBottom: '10px' }}> {t.angelDemon} </h2>
...
export default function Roulette() {
  ...
  const spinWheel = async () => {
    // 1️⃣ [즉시 차단] 함정 카드 발동!
    if (isSpinning || isProcessing.current) return;

    // 2️⃣ [임시 잠금] 검사 전에 먼저 문부터 잠급니다.
    isProcessing.current = true;
    
    if (betMoney <= 0) {
        isProcessing.current = false;
        return alert(t.alertInputBet);
    }
    if (betMoney > Math.floor(point)) {
        isProcessing.current = false;
        return alert(t.alertNoMoney);
    }
    if (!selectedSide) {
        isProcessing.current = false;
        return alert(t.rouletteSelect);
    }
      ...
      <div style={{ display: 'flex', width: '100%', maxWidth: '800px', gap: '20px', flexWrap: 'wrap', justifyContent: 'center' }}>
        ...
        <div style={{ flex: 1, minWidth: '300px', maxWidth: '350px' }}>
          ...
          <button 
            className="btn" 
            onClick={spinWheel} 
            disabled={isSpinning || isProcessing.current} 
            style={{ width: '100%', marginTop: '15px', padding: '15px', background: (isSpinning || isProcessing.current) ? '#7f8c8d' : '#f1c40f', color: '#000', fontSize: '20px', fontWeight: 'bold' }} 
          > 
            {(isSpinning || isProcessing.current) ? t.spinning : "GAME START (x1.97)"} 
          </button>
        </div>
      </div>
      <button className="btn" style={{ marginTop: 20, background: 'transparent', border:'1px solid #555', color:'#888' }} onClick={() => navigate('/home')}> {t.home} </button>
